...
function FlowingMenu({ items = [] }) {
  return (
    <div className="w-full h-full overflow-hidden">
      ...
      <nav className="flex flex-col h-full m-2 p-8">
        {items.map((item, idx) => (
...
function MenuItem({ link, text, image }) {
  ...
  const repeatedMarqueeContent = Array.from({ length: 4 }).map((_, idx) => (
    <Fragment key={idx}>
      <span className="text-[#060010] uppercase font-normal text-[9vh] leading-[1.2] p-[1vh_1vw_0]">
        {text}
      </span>
      <div
    ...
    >
      ...
      >
        <div className="h-full w-[200%] flex" ref={marqueeInnerRef}>
          <div className="flex items-center relative h-full w-[200%] marquee-animation">
